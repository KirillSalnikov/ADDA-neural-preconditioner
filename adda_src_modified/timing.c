...
TIME_TYPE Timing_OneIter,Timing_OneIterComm,       // for one iteration: total & comm
          ...
          Timing_Precond,Timing_OneIterPrecond;    // total & one-iteration time for preconditioner apply
...
// init timing variables and counters
{
	...
	Timing_Precond=Timing_OneIterPrecond=0;
#ifdef SPARSE
...
#	endif
		...
		if (!prognosis) {
			...
			if (Timing_Precond!=0) fprintf(logfile,
				"      preconditioner apply: "FFORMT"\n",TO_SEC(Timing_Precond));
#ifdef PARALLEL
			fprintf(logfile,
			...
			if (Timing_OneIterPrecond!=0) fprintf(logfile,
				"        preconditioner apply: "FFORMT"\n",TO_SEC(Timing_OneIterPrecond));
#ifdef PARALLEL
			fprintf(logfile,
