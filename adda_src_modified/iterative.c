...
extern TIME_TYPE Timing_OneIter,Timing_OneIterComm,Timing_InitIter,Timing_InitIterComm,Timing_IntFieldOneComm,
	Timing_MVP,Timing_MVPComm,Timing_OneIterMVP,Timing_OneIterMVPComm,Timing_Precond,Timing_OneIterPrecond;
...
static bool left_precond;  // use preconditioner inside the iterative method, not only for x0
static bool iter_restart;  // restart Krylov recurrence after external residual correction
	// whether matrix-vector product computed during initialization can be reused at first iteration
static bool matvec_ready;
static bool orient_maxiter_warned=false;
static int precond_periodic_period;
static double precond_periodic_relax;
static double precond_periodic_max_rel_resid;
...
static bool ParseBoolString(const char *value,bool *out)
{
...
bool OrientVerbose(void)
{
	static int verbose=-1;
	bool parsed;

	if (!orient_avg) return true;
	if (verbose<0) {
		verbose=0;
		if (ParseBoolString(getenv("ADDA_ORIENT_VERBOSE"),&parsed)) verbose=parsed ? 1 : 0;
	}
	return verbose!=0;
}
...
static int ParsePositiveIntEnv(const char *name)
{
	...
	if (endptr==value || *endptr!='\0' || parsed<0 || parsed>2147483647)
		LogError(ONE_POS,"Invalid %s=%s; expected a non-negative integer",name,value);
	return (int)parsed;
}

//======================================================================================================================

static double ParsePositiveDoubleEnv(const char *name,double default_value)
{
	const char *value=getenv(name);
	char *endptr;
	double parsed;
	...
	if (endptr==value || *endptr!='\0' || parsed<=0)
		LogError(ONE_POS,"Invalid %s=%s; expected a positive number",name,value);
	return parsed;
}

//======================================================================================================================

void FreeIterativeAux(void)
{
	orient_maxiter_warned=false;
}

//======================================================================================================================

	/* Left preconditioning for BiCGStab: solve M*A*x = M*b.
	 *
	 * All BiCGStab vectors (p, r_hat, s, v, t, rtilda) live in the preconditioned (M*A) space.
 * The solution x lives in the original space and is updated directly: x += alpha*p + omega*s.
 *
 * The actual residual ||b - A*x|| is tracked algebraically using the intermediate A*p and A*s values
 * (computed before applying M), so no extra MatVec is needed per iteration:
 *   r_actual_k = r_actual_{k-1} - alpha*(A*p) - omega*(A*s)
 *
 * For single-material, M = raw SAI preconditioner (PrecondApply).
 * For multi-material, M = S*M_raw*S^{-1} (PrecondApplyScaled) to account for ADDA's S-scaling.
 */
...
static bool ShouldPeriodicPrecondCorrection(void)
{
	if (precond_periodic_period<=0) return false;
	if (left_precond || !use_precond) return false;
	if (!complete || inprodRp1<=epsB) return false;
	return (niter>1 && ((niter-1)%precond_periodic_period)==0);
}

//======================================================================================================================

static void ApplyPeriodicPrecondCorrection(void)
/* Experimental additive correction: x <- x + alpha*M*r. This keeps the ordinary BiCGStab operator unchanged and uses the
 * neural inverse only occasionally. The true residual is recomputed after the trial and the correction is accepted only
 * if it does not make the residual worse beyond the configured ratio.
 */
{
	double old_res,new_res,restored_res,old_err,new_err,trial_err;
	bool accepted;

	old_res=(inprodRp1>0 ? inprodRp1 : inprodR);
	old_err=sqrt(resid_scale*old_res);
	PrecondApply(rvec,precond.tmp,local_nRows);
	nIncrem01_cmplx(xvec,precond.tmp,precond_periodic_relax,NULL,NULL);
	new_res=ResidualNorm2(xvec,rvec,Avecbuffer,&Timing_MVP,&Timing_MVPComm,&Timing_IntFieldOneComm);
	new_err=sqrt(resid_scale*new_res);
	trial_err=new_err;
	accepted=(new_res<=old_res*precond_periodic_max_rel_resid);
	if (accepted) {
		inprodRp1=new_res;
		if (new_res<=inprodR) {
			inprodR=new_res;
		...
		nIncrem01_cmplx(xvec,precond.tmp,-precond_periodic_relax,NULL,NULL);
		restored_res=ResidualNorm2(xvec,rvec,Avecbuffer,&Timing_MVP,&Timing_MVPComm,&Timing_IntFieldOneComm);
		inprodRp1=restored_res;
		new_err=sqrt(resid_scale*restored_res);
	}
	prev_err=new_err;
	if (IFROOT) {
		if (accepted) {
			fprintf(logfile,"Periodic preconditioner correction accepted at iteration %d: "EFORM" -> "EFORM"\n",
				niter-1,old_err,new_err);
			if (OrientVerbose())
				PRINTFB("Periodic preconditioner correction accepted at iteration %d: "EFORM" -> "EFORM"\n",
					niter-1,old_err,new_err);
		}
		else {
			fprintf(logfile,"Periodic preconditioner correction rejected at iteration %d: trial "EFORM" -> "EFORM"\n",
				niter-1,old_err,trial_err);
			if (OrientVerbose())
				PRINTFB("Periodic preconditioner correction rejected at iteration %d: trial "EFORM" -> "EFORM"\n",
					niter-1,old_err,trial_err);
		}
	}
}
...
#define EPS2 1E-10 // for |v.r~|/|r.r~|
	...
	switch (ph) {
		...
		case PHASE_INIT:
			...
			return;
		case PHASE_ITER:
			/* When left_precond: all BiCGStab vectors (rvec=r_hat, pvec=p, v, s) are in preconditioned
			 * (M*A) space. The actual residual ||b-A*x|| is tracked algebraically in precond.r_actual.
			 */
			// ro_k-1=r_k-1.r~ ; check for ro_k-1!=0
			ro_new=nDotProd(rvec,rtilda,&Timing_OneIterComm);
			if (niter==1 || iter_restart) {
				nCopy(pvec,rvec); // p_1=r_0, or restart after external residual correction
				iter_restart=false;
			}
			else {
			...
			// calculate v_k: left precond v=M*A*p, otherwise v=A*p
			if (left_precond) {
				if (PrecondCanApplyMatVecFused()) {
					PrecondApplyMatVecFused(pvec,v,local_nRows);
				}
				else {
					// Step 1: tmp = A*p (save intermediate for actual residual tracking)
					MatVec(pvec,precond.tmp,NULL,false,&Timing_OneIterMVP,&Timing_OneIterMVPComm);
					// Step 2: v = M*(A*p)
					PrecondApply(precond.tmp,v,local_nRows);
				}
			}
			else if (niter==1 && matvec_ready) nCopy(v,Avecbuffer);
			...
			if (left_precond) {
				if (PrecondCanApplyMatVecFused()) {
					...
					nLinComb1_cmplx(s,v,rvec,temp1,NULL,NULL); // s in preconditioned space
					// Track actual residual: r_actual -= alpha*(A*p)
					nIncrem01_cmplx(precond.r_actual,precond.tmp,-alpha,&inprodRp1,&Timing_OneIterComm);
				}
			}
			else nLinComb1_cmplx(s,v,rvec,temp1,&inprodRp1,&Timing_OneIterComm);
			...
			if (inprodRp1<epsB && chp_type!=CHP_ALWAYS) {
				...
				// t=M*A*s (left precond) or t=A*s (no precond)
				if (left_precond) {
					if (PrecondCanApplyMatVecFused()) {
						PrecondApplyMatVecFused(s,Avecbuffer,local_nRows);
					}
					else {
						// Step 1: tmp2 = A*s (save intermediate for actual residual tracking)
						MatVec(s,precond.tmp2,NULL,false,&Timing_OneIterMVP,&Timing_OneIterMVPComm);
						// Step 2: Avecbuffer = M*(A*s) = t
						PrecondApply(precond.tmp2,Avecbuffer,local_nRows);
					}
					// Compute |t|^2 for omega
					denumOmega=nNorm2(Avecbuffer,&Timing_OneIterComm);
				}
				else MatVec(s,Avecbuffer,&denumOmega,false,&Timing_OneIterMVP,&Timing_OneIterMVPComm);
				// omega_k=s.t/|t|^2
				...
				// r_hat=s-omega*t (preconditioned residual update)
				temp1=-omega;
				if (left_precond) {
					if (PrecondCanApplyMatVecFused()) {
						...
						nLinComb1_cmplx(rvec,Avecbuffer,s,temp1,NULL,NULL);
						// Track actual residual: r_actual -= omega*(A*s)
						nIncrem01_cmplx(precond.r_actual,precond.tmp2,-omega,&inprodRp1,
							&Timing_OneIterComm);
					}
				}
				else nLinComb1_cmplx(rvec,Avecbuffer,s,temp1,&inprodRp1,&Timing_OneIterComm);
				// initialize ro_old -> ro_k-2 for next iteration
				ro_old=ro_new;
			}
			return; // end of PHASE_ITER
...
static double FGMRESLeastSquares(const int k,const double beta,const doublecomplex * restrict H,const int ld,
	doublecomplex * restrict y,doublecomplex * restrict A,doublecomplex * restrict b,doublecomplex * restrict Hy)
/* Solve the small least-squares problem min || beta*e_1 - H_k*y || via normal equations. The work arrays are reused
 * across Arnoldi steps; this was the fastest tested variant for the current large-grid FFTDIRECT workflow.
 */
{
	int i,j,l,p,piv;
	doublecomplex factor,sum;
	double max_abs,piv_abs;
	double res2;

	for (i=0;i<k;i++) {
		for (j=0;j<k;j++) {
			sum=0;
			for (l=0;l<=k;l++) sum += conj(H[l+i*ld])*H[l+j*ld];
			A[i*k+j]=sum;
		}
		b[i]=conj(H[0+i*ld])*beta;
	}
	for (p=0;p<k;p++) {
		piv=p;
		max_abs=cabs(A[p*k+p]);
		for (i=p+1;i<k;i++) {
			piv_abs=cabs(A[i*k+p]);
			if (piv_abs>max_abs) {
				max_abs=piv_abs;
				piv=i;
			}
		...
		for (i=p+1;i<k;i++) {
			factor=A[i*k+p]/A[p*k+p];
			A[i*k+p]=0;
			for (j=p+1;j<k;j++) A[i*k+j]-=factor*A[p*k+j];
			b[i]-=factor*b[p];
		}
	}
	for (i=k-1;i>=0;i--) {
		sum=b[i];
		for (j=i+1;j<k;j++) sum-=A[i*k+j]*y[j];
		y[i]=sum/A[i*k+i];
	}
	res2=0;
	for (i=0;i<=k;i++) {
		sum=0;
		for (j=0;j<k;j++) sum+=H[i+j*ld]*y[j];
		Hy[i]=sum;
		if (i==0) Hy[i]-=beta;
		res2+=cAbs2(Hy[i]);
	}
...
ITER_FUNC(FGMRES)
/* Experimental restarted flexible GMRES with right preconditioning. With -precond, each Arnoldi step applies z=M*v and
 * then w=A*z, so it uses one preconditioner application per step instead of two in BiCGStab.
 */
{
	static int restart_dim=0,jcur=0,alloc_restart=0;
	static size_t alloc_n=0;
	static double beta=0;
	static doublecomplex *Vblock=NULL,*Zblock=NULL,*H=NULL,*ycoef=NULL,*ls_A=NULL,*ls_b=NULL,*ls_Hy=NULL;
	int env_restart,k,i;
	double hnext;
	bool finalize;

#define FGMRES_V(i) (Vblock+(size_t)(i)*local_nRows)
#define FGMRES_Z(i) (Zblock+(size_t)(i)*local_nRows)
#define FGMRES_H(row,col) H[(row)+(col)*(restart_dim+1)]

	switch (ph) {
		case PHASE_VARS:
			env_restart=ParsePositiveIntEnv("ADDA_FGMRES_RESTART");
			restart_dim=(env_restart>0 ? env_restart : 30);
			if (restart_dim<2 || restart_dim>200)
				LogError(ONE_POS,"Invalid ADDA_FGMRES_RESTART=%d; expected 2..200",restart_dim);
			if (Vblock==NULL || alloc_n!=local_nRows || alloc_restart!=restart_dim) {
				if (Vblock!=NULL) {
					free(Vblock);
					free(Zblock);
					free(H);
				...
				alloc_n=local_nRows;
				alloc_restart=restart_dim;
				Vblock=(doublecomplex *)voidVector((size_t)(restart_dim+1)*local_nRows*sizeof(doublecomplex),
					ALL_POS,"FGMRES V basis");
				Zblock=(doublecomplex *)voidVector((size_t)restart_dim*local_nRows*sizeof(doublecomplex),
					ALL_POS,"FGMRES Z basis");
				H=(doublecomplex *)voidVector((size_t)(restart_dim+1)*(size_t)restart_dim*sizeof(doublecomplex),
					ALL_POS,"FGMRES Hessenberg");
				ycoef=(doublecomplex *)voidVector((size_t)restart_dim*sizeof(doublecomplex),ALL_POS,
					"FGMRES coefficients");
				ls_A=(doublecomplex *)voidVector((size_t)restart_dim*(size_t)restart_dim*sizeof(doublecomplex),
					ALL_POS,"FGMRES least-squares matrix");
				ls_b=(doublecomplex *)voidVector((size_t)restart_dim*sizeof(doublecomplex),ALL_POS,
					"FGMRES least-squares RHS");
				ls_Hy=(doublecomplex *)voidVector((size_t)(restart_dim+1)*sizeof(doublecomplex),ALL_POS,
					"FGMRES least-squares residual");
			}
			return;
		case PHASE_INIT:
			beta=sqrt(inprodR);
			jcur=0;
			memset(H,0,(size_t)(restart_dim+1)*(size_t)restart_dim*sizeof(doublecomplex));
			if (beta>0) nMult(FGMRES_V(0),rvec,1.0/beta);
			if (IFROOT && niter==1)
				fprintf(logfile,"FGMRES restart dimension: %d%s\n",restart_dim,
					(use_precond && !precond_init_only) ? ", right-preconditioned" : "");
			return;
		case PHASE_ITER:
			if (beta<=0) {
				inprodRp1=0;
			...
			if (use_precond && !precond_init_only) PrecondApply(FGMRES_V(jcur),FGMRES_Z(jcur),local_nRows);
			else nCopy(FGMRES_Z(jcur),FGMRES_V(jcur));
			MatVec(FGMRES_Z(jcur),Avecbuffer,NULL,false,&Timing_OneIterMVP,&Timing_OneIterMVPComm);
			for (i=0;i<=jcur;i++) {
				FGMRES_H(i,jcur)=nDotProd(Avecbuffer,FGMRES_V(i),&Timing_OneIterComm);
				nIncrem01_cmplx(Avecbuffer,FGMRES_V(i),-FGMRES_H(i,jcur),NULL,NULL);
			}
			hnext=sqrt(nNorm2(Avecbuffer,&Timing_OneIterComm));
			FGMRES_H(jcur+1,jcur)=hnext;
			if (hnext>1e-300 && jcur+1<restart_dim) nMult(FGMRES_V(jcur+1),Avecbuffer,1.0/hnext);
			k=jcur+1;
			inprodRp1=FGMRESLeastSquares(k,beta,H,restart_dim+1,ycoef,ls_A,ls_b,ls_Hy);
			finalize=(inprodRp1<epsB || k==restart_dim || hnext<=1e-300);
			if (finalize) {
				for (i=0;i<k;i++) nIncrem01_cmplx(xvec,FGMRES_Z(i),ycoef[i],NULL,NULL);
				inprodRp1=ResidualNorm2(xvec,rvec,Avecbuffer,&Timing_MVP,&Timing_MVPComm,&Timing_IntFieldOneComm);
				if (inprodRp1>epsB) {
					beta=sqrt(inprodRp1);
					jcur=0;
					memset(H,0,(size_t)(restart_dim+1)*(size_t)restart_dim*sizeof(doublecomplex));
 ...
 */
{
	...
	TIME_TYPE tstart,time_tmp,time_tmp2,time_tmp3,time_tmp4;

	// redundant initialization to remove warnings
	time_tmp=time_tmp2=time_tmp3=time_tmp4=0;
	 ...
	 */
	Timing_InitIterComm=Timing_MVP=Timing_MVPComm=Timing_Precond=0;
	tstart=GET_TIME();
	precond_periodic_period=ParsePositiveIntEnv("ADDA_PRECOND_PERIODIC_CORRECTION");
	precond_periodic_relax=ParsePositiveDoubleEnv("ADDA_PRECOND_PERIODIC_RELAX",1.0);
	precond_periodic_max_rel_resid=ParsePositiveDoubleEnv("ADDA_PRECOND_PERIODIC_MAX_REL_RESID",1.0);
	if (precond_periodic_period>0) {
		if (!use_precond)
			LogError(ONE_POS,"ADDA_PRECOND_PERIODIC_CORRECTION requires a preconditioner file (-precond)");
		if (precond_init_only)
			LogError(ONE_POS,"ADDA_PRECOND_PERIODIC_CORRECTION is incompatible with ADDA_PRECOND_INIT_ONLY");
		if (method_in!=IT_BICGSTAB)
			LogError(ONE_POS,"ADDA_PRECOND_PERIODIC_CORRECTION is implemented only for -iter bicgstab");
		if (load_chpoint)
			LogError(ONE_POS,"ADDA_PRECOND_PERIODIC_CORRECTION is not supported with iteration checkpoints");
	}
	if (method_in==IT_FGMRES && (load_chpoint || chp_type!=CHP_NONE))
		LogError(ONE_POS,"FGMRES iteration checkpoints are not implemented");
	left_precond=use_precond && !precond_init_only && precond_periodic_period==0 && method_in==IT_BICGSTAB;
	iter_restart=false;
	...
	if (!load_chpoint) {
		...
		if (use_precond && precond_init_only) {
			// One-shot initial guess: x_0 <- x_0 + M*r_0, then recompute the true residual.
			PrecondApply(rvec,precond.tmp,local_nRows);
			nIncrem01_cmplx(xvec,precond.tmp,1,NULL,NULL);
			MatVec(xvec,Avecbuffer,NULL,false,&Timing_MVP,&Timing_MVPComm);
			nSubtr(rvec,pvec,Avecbuffer,&inprodR,&Timing_InitIterComm);
			matvec_ready=false;
			descr=dyn_sprintf("%s; x_0 += M*r_0",descr);
		}
		// Left preconditioning: save actual residual, then transform rvec to preconditioned space
		if (left_precond) {
			// Save r_0 = b - A*x_0 as actual residual for algebraic tracking
			memcpy(precond.r_actual,rvec,local_nRows*sizeof(doublecomplex));
			// Transform rvec to preconditioned space: rvec = M * r_0
			PrecondApply(rvec,precond.tmp,local_nRows);
			memcpy(rvec,precond.tmp,local_nRows*sizeof(doublecomplex));
			// inprodR stays as |r_0|^2 (actual residual), epsB stays based on |b|^2
			matvec_ready=false;
		}
		// print start values
		if (IFROOT) {
	...
	while (inprodR>epsB && niter<=maxiter && counter<=params[ind_m].mc && !chp_exit) {
		// initialize time
		Timing_OneIterComm=Timing_OneIterMVP=Timing_OneIterMVPComm=Timing_OneIterPrecond=0;
		tstart=GET_TIME();
		// main execution
		...
		if (complete) {
			...
			time_tmp4=Timing_OneIterPrecond;
		}
		// use result from the previous iteration (assumed to be available by this time)
			...
			Timing_OneIterPrecond=time_tmp4;
		}
		 ...
		 */
		ProgressReport();
		if (ShouldPeriodicPrecondCorrection()) ApplyPeriodicPrecondCorrection();
	}
	...
	if (inprodR>epsB) {
		if (niter>maxiter) {
			if (!orient_avg || OrientVerbose() || !orient_maxiter_warned) {
				LogWarning(EC_WARN,ONE_POS,"Iterations haven't converged in %d iterations. Further calculated "
					"scattering quantities may be less accurate.",maxiter);
				if (orient_avg && !OrientVerbose()) orient_maxiter_warned=true;
			}
		}
	...
	if (recalc_resid) { // compute and print final residual norm
		...
		if (IFROOT) {
			...
			if (!orient_avg) fprintf(logfile,"%s",tmp_str);
				if (OrientVerbose()) PRINTFB("%s",tmp_str);
			}
	}
	// post-processing
